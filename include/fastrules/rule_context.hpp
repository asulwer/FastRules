...
public:
    ...
    // Store result of a rule execution (by name)
    ...
    // Check if a rule has been executed in this context (by name)
...
private:
    ...
    std::optional<std::pair<std::string, std::string>> lastError_; // {ruleName, errorMessage}
};
