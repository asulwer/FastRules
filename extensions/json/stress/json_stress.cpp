// json_stress.cpp
// Stress scenarios for the JSON extension.
...
std::filesystem::path tempRepoPath(const std::string& name) {
    return std::filesystem::temp_directory_path() / ("fastrules-stress-json-" + name + ".json");
}

static StressResult jsonLoadExecuteRoundtrip(const StressConfig& cfg) {
    ...
    std::string json;
    StressRunner runner;
    return runner.run("json load/execute roundtrip", cfg, [&](size_t i) {
        json = JsonLoader::saveWorkflow(wf);
        auto loaded = JsonLoader::loadWorkflow(json);
...
static StressResult jsonRuleRepositoryChurn(const StressConfig& cfg) {
    auto path = tempRepoPath("rules");
    std::filesystem::remove(path);
    JsonRuleRepository repo(path);

    // Pre-populate a set of rules.
    ...
    StressRunner runner;
    return runner.run("json rule repository churn", cfg, [&](size_t i) {
...
int main(int argc, char* argv[]) {
    std::vector<std::function<StressResult(const StressConfig&)>> scenarios = {
        jsonLoadExecuteRoundtrip,
        jsonRuleRepositoryChurn,
    };
    return runSuite("json", scenarios, argc, argv);
}
