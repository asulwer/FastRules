...
void DbRuleRepository::insertRule(const Rule& rule) {
    ...
    *session_ << 
        "INSERT INTO rules (id, name, expression, action, description, is_active, priority, timeout_ms, cache_duration_ms, depends_on_rule_name) "
        "VALUES (:id, :name, :expr, :act, :desc, :active, :prio, :timeout, :cache, :dep)",
        soci::use(rule.id), soci::use(rule.name), soci::use(rule.expression), soci::use(rule.action), soci::use(rule.description),
...
void DbRuleRepository::updateRule(const Rule& rule) {
    ...
    *session_ <<
        "UPDATE rules SET "
        "  name = :name, expression = :expr, action = :act, description = :desc, is_active = :active, "
        "  priority = :prio, timeout_ms = :timeout, cache_duration_ms = :cache, depends_on_rule_name = :dep, "
        "  updated_at = CURRENT_TIMESTAMP "
        "WHERE id = :id",
        soci::use(rule.name), soci::use(rule.expression), soci::use(rule.action), soci::use(rule.description),
...
std::vector<Rule> DbRuleRepository::findAll() {
    ...
    soci::rowset<soci::row> rs = ((*session_).prepare <<
        "SELECT id, name, expression, action, description, is_active, priority, timeout_ms, cache_duration_ms, depends_on_rule_name FROM rules");
...
void DbWorkflowRepository::save(const Workflow& workflow) {
    ...
    int count = 0;
    *session_ << "SELECT COUNT(*) FROM workflows WHERE id = :id", soci::into(count), soci::use(workflow.id);
...
std::optional<Workflow> DbWorkflowRepository::findById(int id) {
    ...
    if (outerIt != outerEnd) {
        ...
        soci::rowset<soci::row> ruleRs = ((*session_).prepare <<
            ...
            "       r.timeout_ms, r.cache_duration_ms, r.depends_on_rule_name "
            "FROM rules r "
...
std::shared_ptr<soci::session> DbConnectionFactory::create(
    ...
    // Use the statically-linked SQLite3 backend factory directly when the
    // SQLite backend is requested. This avoids relying on SOCI's dynamic
    // backend loader, which can fail to locate the plugin DLL on Windows.
    if (backend == "sqlite3") {
        return std::make_shared<soci::session>(*soci::factory_sqlite3(), connectionString);
    }
