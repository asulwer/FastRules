...
[[maybe_unused]] static int lua_sleep(lua_State* L) {
...
TEST_CASE("Performance: parallel vs sequential") {
    ...
    SUBCASE("Compare execution times") {
        ...
        auto seqDuration [[maybe_unused]] = std::chrono::duration_cast<std::chrono::milliseconds>(seqEnd - start).count();

        start = std::chrono::steady_clock::now();
        auto parResults = workflow.executeParallel(engine, {});
        auto parEnd = std::chrono::steady_clock::now();
        auto parDuration [[maybe_unused]] = std::chrono::duration_cast<std::chrono::milliseconds>(parEnd - start).count();
